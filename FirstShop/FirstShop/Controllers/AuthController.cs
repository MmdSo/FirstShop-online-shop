...
namespace FirstShop.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public AuthController(IConfiguration config, AppDbContext context, IUserServices userServices)
        {
            _config = config;
            _context = context;
        ...
        private Data.RefreshTokens.RefrshToken GenerateRefreshToken(long userId)
        {
            return new Data.RefreshTokens.RefrshToken
            {
        ...
        public IActionResult LoginJWT([FromQuery] LoginViewModel login)
        {
            ...
            if (login.UserName != null && pass != null)
            {
                ...
                var refreshToken = new RefrshToken
                {
                    Token = Guid.NewGuid().ToString("N"),
                    UserId = user.id,
                    Expires = DateTime.UtcNow.AddDays(7),
